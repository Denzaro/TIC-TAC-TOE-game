...
void GUI_option::show_GUI_Option(QMainWindow *window){
    ...
    option_Window->show();
    connect(optionUI->sound_3,QPushButton::clicked,gameAgain,&GameInterface::stopMusic);
    //cấu hình nút new
...
void GameInterface::music_game(){
    ...
    player->play();
    connect(game_ui.sound_4, &QPushButton::clicked, this, &GameInterface::stopMusic);
}

void GameInterface::toggleState(bool &state)
{
    state = !state;
}
 // Ví dụ về biến boolean cần đảo trạng thái
bool musicPlaying = true;
// Sử dụng hàm toggleState() để đảo ngược trạng thái của biến musicPlaying

void GameInterface::stopMusic()
{
    // Kiểm tra trạng thái của âm thanh và dừng phát nếu đang phát
    toggleState(musicPlaying); // Đảo ngược trạng thái của biến musicPlaying

    // Kiểm tra và điều khiển phát nhạc dựa trên trạng thái mới
    if (musicPlaying) {
        player->play(); // Phát nhạc nếu đang tắt
    } else {
        player->stop(); // Dừng nhạc nếu đang phát
    }
